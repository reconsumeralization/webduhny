...
export class SecurityApiKeysMigrator {
    ...
    async run() {
        ...
        for (const apiKey of sourceListApiKeys.data) {
            const currentlyUsedSourceApiKey = apiKey.token === this.securityMigrator.sourceApiKey;
            if (currentlyUsedSourceApiKey) {
                continue;
            }
            ...
            if (error) {
                console.log(`Failed to migrate API key "${apiKey.title}". Error:`, error);
            }
        }
    }
}
