...
interface IGetPackagesParams {
    inputs: IUserCommandInput;
    output: SimpleOutput;
    context: Pick<Context, "debug" | "warning" | "error">;
}
...
const getPackages = async ({ inputs, context, output }: IGetPackagesParams) => {
    ...
    if (inputs.package) {
        ...
        packagesList = await execa("yarn", cmd).then(({ stdout }) => JSON.parse(stdout));
    }
...
const createLog = ({ multipleWatches, output, context }: ICreateLogParams) => {
    return (packageName: string, message: string | string[], type?: "error" | "warn") => {
        let prefix = "";
        if (multipleWatches) {
