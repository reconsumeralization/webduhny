...
const getCategoryByIdQuery = (model: CmsModel) => {
    return /* GraphQL */ `
        query GetCategory($id: ID!) {
            getCategory: get${model.singularApiName}ById(id: $id) {
                data {
                    ${categoryFields}
                }
                ${errorFields}
...
export const useCategoryPreviewHandler = (params: GraphQLHandlerParams) => {
    ...
    return {
        ...
        async getCategoryById(id: string, headers: Record<string, any> = {}) {
            return await contentHandler.invoke({
                body: {
                    query: getCategoryByIdQuery(model),
                    variables: {
                        id
                    }
                },
